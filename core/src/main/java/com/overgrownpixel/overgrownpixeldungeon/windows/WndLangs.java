...
public class WndLangs extends Window {
	...
	public WndLangs(){
		...
		for (int i = 0; i < langs.size(); i++){
				...
				protected void onClick() {
					...
					OvergrownPixelDungeon.seamlessResetScene(new Game.SceneChangeCallback() {
						...
						public void beforeCreate() {
							OGPDSettings.language(langs.get(langIndex));
			...
			if (OGPDSettings.landscape() && i % 2 == 1){
				btn.setPos(BTN_WIDTH+1, y-(BTN_HEIGHT + 1));
			} else {
				btn.setPos(0, y);
				y += BTN_HEIGHT;
				if (OGPDSettings.landscape()) y++;
			}

			add(btn);
		}
		y = Math.max(MIN_HEIGHT, y);
		resize(OGPDSettings.landscape() ? WIDTH_L : WIDTH_P, y);
		...
		if (currLang == Languages.ENGLISH){
			...
			RedButton creditsBtn = new RedButton(Messages.titleCase(Messages.get(this, "credits"))){
				...
				protected void onClick() {
					...
					boolean wide = false;
					if (OGPDSettings.landscape() && (reviewers.length + translators.length) > 10){
						wide = true;
					}
