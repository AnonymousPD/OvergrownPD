		...
		public DisplayTab() {
			...
			OptionSlider scale = new OptionSlider(Messages.get(this, "scale"),
				...
				protected void onChange() {
					if (getSelectedValue() != OGPDSettings.scale()) {
						OGPDSettings.scale(getSelectedValue());
						OvergrownPixelDungeon.seamlessResetScene();
					}
				}
			};
					...
					if (checked()) {
						...
						OvergrownPixelDungeon.scene().add(new WndOptions(
							...
							protected void onSelect(int index) {
								if (index == 0) {
									checked(!checked());
									OGPDSettings.powerSaver(checked());
								}
							}
						});
					} else {
			...
			if (PixelScene.maxScreenZoom >= 2) {
				...
				chkSaver.checked(OGPDSettings.powerSaver());
				add(chkSaver);
			}

			RedButton btnOrientation = new RedButton( OGPDSettings.landscape() ?
			...
			OptionSlider brightness = new OptionSlider(Messages.get(this, "brightness"),
				...
				@Override
				protected void onChange() {
					OGPDSettings.brightness(getSelectedValue());
				}
			};
			brightness.setSelectedValue(OGPDSettings.brightness());
			...
			OptionSlider tileGrid = new OptionSlider(Messages.get(this, "visual_grid"),
				...
				@Override
				protected void onChange() {
					OGPDSettings.visualGrid(getSelectedValue());
				}
			};
			tileGrid.setSelectedValue(OGPDSettings.visualGrid());
	...
	private class UITab extends Group {
		...
		public UITab(){
			...
			RedButton btnSplit = new RedButton(Messages.get(this, "split")){
				@Override
				protected void onClick() {
					OGPDSettings.toolbarMode(Toolbar.Mode.SPLIT.name());
			...
			RedButton btnGrouped = new RedButton(Messages.get(this, "group")){
				@Override
				protected void onClick() {
					OGPDSettings.toolbarMode(Toolbar.Mode.GROUP.name());
					Toolbar.updateLayout();
				}
			};
			...
			RedButton btnCentered = new RedButton(Messages.get(this, "center")){
				@Override
				protected void onClick() {
					OGPDSettings.toolbarMode(Toolbar.Mode.CENTER.name());
					Toolbar.updateLayout();
				}
			};
			...
			CheckBox chkFlipToolbar = new CheckBox(Messages.get(this, "flip_toolbar")){
				...
				protected void onClick() {
					super.onClick();
					OGPDSettings.flipToolbar(checked());
					Toolbar.updateLayout();
				}
			};
			chkFlipToolbar.setRect(0, btnGrouped.bottom() + GAP_TINY, WIDTH, BTN_HEIGHT);
			chkFlipToolbar.checked(OGPDSettings.flipToolbar());
			add(chkFlipToolbar);
			...
			final CheckBox chkFlipTags = new CheckBox(Messages.get(this, "flip_indicators")){
				...
				protected void onClick() {
					super.onClick();
					OGPDSettings.flipTags(checked());
					GameScene.layoutTags();
				}
			};
			chkFlipTags.setRect(0, chkFlipToolbar.bottom() + GAP_TINY, WIDTH, BTN_HEIGHT);
			chkFlipTags.checked(OGPDSettings.flipTags());
			add(chkFlipTags);

			OptionSlider slots = new OptionSlider(Messages.get(this, "quickslots"), "0", "4", 0, 4) {
				@Override
				protected void onChange() {
					OGPDSettings.quickSlots(getSelectedValue());
					Toolbar.updateLayout();
				}
			};
			slots.setSelectedValue(OGPDSettings.quickSlots());
			...
			CheckBox chkImmersive = new CheckBox( Messages.get(this, "nav_bar") ) {
				...
				protected void onClick() {
					super.onClick();
					OGPDSettings.fullscreen(checked());
				}
			};
			chkImmersive.setRect( 0, slots.bottom() + GAP_SML, WIDTH, BTN_HEIGHT );
			chkImmersive.checked(OGPDSettings.fullscreen());
			...
			CheckBox chkFont = new CheckBox(Messages.get(this, "system_font")){
				...
				protected void onClick() {
					super.onClick();
					OvergrownPixelDungeon.seamlessResetScene(new Game.SceneChangeCallback() {
						@Override
						public void beforeCreate() {
							OGPDSettings.systemFont(checked());
						}
			...
			chkFont.checked(OGPDSettings.systemFont());
			add(chkFont);
		}
	...
	private class AudioTab extends Group {
		...
		public AudioTab() {
			OptionSlider musicVol = new OptionSlider(Messages.get(this, "music_vol"), "0", "10", 0, 10) {
				@Override
				protected void onChange() {
					OGPDSettings.musicVol(getSelectedValue());
				}
			};
			musicVol.setSelectedValue(OGPDSettings.musicVol());
			...
			CheckBox musicMute = new CheckBox(Messages.get(this, "music_mute")){
				...
				protected void onClick() {
					super.onClick();
					OGPDSettings.music(!checked());
				}
			};
			musicMute.setRect(0, musicVol.bottom() + GAP_TINY, WIDTH, BTN_HEIGHT);
			musicMute.checked(!OGPDSettings.music());
			add(musicMute);
			...
			OptionSlider SFXVol = new OptionSlider(Messages.get(this, "sfx_vol"), "0", "10", 0, 10) {
				@Override
				protected void onChange() {
					OGPDSettings.SFXVol(getSelectedValue());
				}
			};
			SFXVol.setSelectedValue(OGPDSettings.SFXVol());
			...
			CheckBox btnSound = new CheckBox( Messages.get(this, "sfx_mute") ) {
				...
				protected void onClick() {
					super.onClick();
					OGPDSettings.soundFx(!checked());
					Sample.INSTANCE.play( Assets.SND_CLICK );
				}
			};
			btnSound.setRect(0, SFXVol.bottom() + GAP_TINY, WIDTH, BTN_HEIGHT);
			btnSound.checked(!OGPDSettings.soundFx());
			add( btnSound );
