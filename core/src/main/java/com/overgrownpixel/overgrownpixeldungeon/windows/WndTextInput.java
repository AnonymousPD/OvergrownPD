...
public class WndTextInput extends Window {
	...
	public WndTextInput(final String title, final String initialValue, final int maxLength,
		...
		final int width;
		if (OGPDSettings.landscape() && multiLine){
		...
		OvergrownPixelDungeon.instance.runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				textInput = new EditText(OvergrownPixelDungeon.instance);
				...
				OvergrownPixelDungeon.instance.addContentView(textInput, layout);
			}
		});
	}
		...
		if (textInput != null){
			OvergrownPixelDungeon.instance.runOnUiThread(new Runnable() {
				...
				public void run() {
					...
					InputMethodManager imm = (InputMethodManager) OvergrownPixelDungeon
