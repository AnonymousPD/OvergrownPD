...
import com.overgrownpixel.overgrownpixeldungeon.scenes.IntroScene;
	...
	public WndStartGame(final int slot){
		...
		RedButton start = new RedButton(Messages.get(this, "start")){
			...
			protected void onClick() {
				...
				if (OGPDSettings.intro()) {
					OGPDSettings.intro( false );
		...
		if (DeviceCompat.isDebug() || Badges.isUnlocked(Badges.Badge.VICTORY)){
			IconButton challengeButton = new IconButton(
					Icons.get( OGPDSettings.challenges() > 0 ? Icons.CHALLENGE_ON :Icons.CHALLENGE_OFF)){
				@Override
				protected void onClick() {
					OvergrownPixelDungeon.scene().add(new WndChallenges(OGPDSettings.challenges(), true) {
						public void onBackPressed() {
							super.onBackPressed();
							icon( Icons.get( OGPDSettings.challenges() > 0 ?
	...
	private class HeroPane extends Component {
		...
		protected void createChildren() {
			...
			heroItem = new IconButton(){
				...
				protected void onClick() {
					if (cl == null) return;
					OvergrownPixelDungeon.scene().add(new WndMessage(Messages.get(cl, cl.name() + "_desc_item")));
				}
			};
			...
			heroLoadout = new IconButton(){
				...
				protected void onClick() {
					if (cl == null) return;
					OvergrownPixelDungeon.scene().add(new WndMessage(Messages.get(cl, cl.name() + "_desc_loadout")));
				}
			};
			...
			heroMisc = new IconButton(){
				...
				protected void onClick() {
					if (cl == null) return;
					OvergrownPixelDungeon.scene().add(new WndMessage(Messages.get(cl, cl.name() + "_desc_misc")));
				}
			};
			...
			heroSubclass = new IconButton(new ItemSprite(ItemSpriteSheet.MASTERY, null)){
				...
				protected void onClick() {
					...
					OvergrownPixelDungeon.scene().add(new WndMessage(msg));
				}
			};
