...
public class WndDocument extends Window {
	...
	public WndDocument( Document doc ){
		...
		int w = OGPDSettings.landscape() ? WIDTH_L : WIDTH_P;
		int h = OGPDSettings.landscape() ? HEIGHT_L : HEIGHT_P;
	...
	private static class docPage extends ListItem {
		...
		public boolean onClick( float x, float y ) {
			if (inside( x, y ) && found) {
				OvergrownPixelDungeon.scene().addToFront( new WndStory( doc.pageBody(page) ));
				return true;
			} else {
				return false;
			}
