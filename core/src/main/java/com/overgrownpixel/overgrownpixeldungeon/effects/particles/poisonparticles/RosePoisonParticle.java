...
public class RosePoisonParticle extends PixelParticle {

    public static final Factory FACTORY = new Factory() {
        @Override
        public void emit(Emitter emitter, int index, float x, float y ) {
            ((RosePoisonParticle)emitter.recycle( RosePoisonParticle.class )).reset( x, y );
        }
        @Override
    ...
    public RosePoisonParticle() {
        size(2.0f);
        color(Game.instance.getResources().getInteger(R.integer.rosepoisonparticle));
