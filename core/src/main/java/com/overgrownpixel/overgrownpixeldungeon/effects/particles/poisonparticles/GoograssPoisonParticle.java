...
public class GoograssPoisonParticle extends PixelParticle {

    public static final Factory FACTORY = new Factory() {
        @Override
        public void emit(Emitter emitter, int index, float x, float y ) {
            ((GoograssPoisonParticle)emitter.recycle( GoograssPoisonParticle.class )).reset( x, y );
        }
        @Override
    ...
    public GoograssPoisonParticle() {
        size(2.0f);
        color(Game.instance.getResources().getInteger(R.integer.goograsspoisonparticle));
