...
public class GobgrapePoisonParticle extends PixelParticle {

    public static final Factory FACTORY = new Factory() {
        @Override
        public void emit(Emitter emitter, int index, float x, float y ) {
            ((GobgrapePoisonParticle)emitter.recycle( GobgrapePoisonParticle.class )).reset( x, y );
        }
        @Override
    ...
    public GobgrapePoisonParticle() {
        size(2.0f);
        color(Game.instance.getResources().getInteger(R.integer.gobgrapepoisonparticle));
