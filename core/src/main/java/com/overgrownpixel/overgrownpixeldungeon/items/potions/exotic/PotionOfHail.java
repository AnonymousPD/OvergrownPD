    ...
    public void apply(Hero hero) {
        ...
        for(Mob mob : hero.visibleEnemiesList().toArray(new Mob[0])){
            if(!(mob instanceof NPC) && !mob.isImmune(this.getClass())){
                Buff.prolong(mob, Frost.class, Frost.duration(mob)*5);
                mob.damage(3, this);
                mob.sprite.burst(Game.instance.getResources().getInteger(R.integer.iceblock), 5);
                mobsHailed++;
            }
        }
        if(mobsHailed > 0){
            GLog.i(Messages.get(this, "mobs_effected", mobsHailed));
        } else {
