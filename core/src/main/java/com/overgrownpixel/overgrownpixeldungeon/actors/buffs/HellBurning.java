...
import com.overgrownpixel.overgrownpixeldungeon.items.food.ChargrilledMeat;
...
public class HellBurning extends Buff implements Hero.Doom {
	...
	public boolean act() {
		...
		if (Dungeon.level.flamable[target.pos] && Blob.volumeAt(target.pos, HellFire.class) == 0) {
			GameScene.add( Blob.seed( target.pos, 4, HellFire.class ) );
		}
	...
	@Override
	public int icon() {
		return BuffIndicator.HELLFIRE;
	}

	@Override
	public void fx(boolean on) {
		if (on) target.sprite.add(CharSprite.State.HELLBURNING);
		else target.sprite.remove(CharSprite.State.HELLBURNING);
	}
