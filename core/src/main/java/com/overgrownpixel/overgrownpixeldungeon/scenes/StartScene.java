...
import com.overgrownpixel.overgrownpixeldungeon.windows.WndGameInProgress;
...
public class StartScene extends PixelScene {
	...
	public void create() {
		...
		int slotGap = OGPDSettings.landscape() ? 5 : 10;
		...
		if (OGPDSettings.landscape()) yPos += 8;
	...
	private static class SaveSlotButton extends Button {
		...
		protected void onClick() {
			if (newGame) {
				OvergrownPixelDungeon.scene().add( new WndStartGame(slot));
			} else {
				OvergrownPixelDungeon.scene().add( new WndGameInProgress(slot));
			}
		}
	}
}
