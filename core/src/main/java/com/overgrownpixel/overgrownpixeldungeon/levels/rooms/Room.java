...
package com.overgrownpixel.overgrownpixeldungeon.levels.rooms;
...
public abstract class Room extends Rect implements Graph.Node, Bundlable {
    ...
    public boolean canPlaceflora(Point p){
        return inside(p);
    }

    public final ArrayList<Point> floraPlaceablePoints(){
        ArrayList<Point> points = new ArrayList<>();
        for (int i = left; i <= right; i++) {
            for (int j = top; j <= bottom; j++) {
                Point p = new Point(i, j);
                if (canPlaceflora(p)) points.add(p);
            }
        }
        return points;
    }
