...
public class Toolbar extends Component {
	...
	protected void layout() {
		...
		int slots = OGPDSettings.quickSlots();
		...
		for(int i = 0; i <= 3; i++) {
			...
			if (slots == 4 && width < 152){
				if (width < 138){
					if ((OGPDSettings.flipToolbar() && i == 3) ||
							(!OGPDSettings.flipToolbar() && i == 0)) {
					...
					if (i == 0 && !OGPDSettings.flipToolbar() ||
						i == 3 && OGPDSettings.flipToolbar()){
						btnQuick[i].border(0, 2);
						btnQuick[i].frame(106, 0, 19, 24);
					} else if (i == 0 && OGPDSettings.flipToolbar() ||
							i == 3 && !OGPDSettings.flipToolbar()){
		...
		float right = width;
		switch(Mode.valueOf(OGPDSettings.toolbarMode())){
			case SPLIT:
				btnWait.setPos(x, y);
